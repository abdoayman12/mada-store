...
import { FiPlus, FiEdit2, FiTrash2, FiAlertCircle, FiTag } from "react-icons/fi";
import { getAdminCategories, deleteCategory } from "@/lib/adminData";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-[#666C5E]">{categories.length} فئة مسجلة</p>
        <Link
      ...
      {categories.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-3xl bg-white py-16 text-center shadow-soft">
          ...
          <p className="font-display text-lg font-bold text-[#2A2E26]">لا توجد فئات بعد</p>
          <p className="text-sm text-[#9A9F8F]">ابدأ بإضافة أول فئة للمتجر</p>
        </div>
      ) : (
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-3xl bg-white p-7 shadow-lift">
            ...
            <h3 className="mt-4 font-display text-lg font-bold text-[#2A2E26]">حذف الفئة؟</h3>
            <p className="mt-2 text-sm text-[#666C5E]">
              هتتحذف الفئة نهائيًا. المنتجات المرتبطة بيها هتفضل موجودة لكن من غير فئة.
            </p>
