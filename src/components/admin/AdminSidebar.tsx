  ...
  return (
    <aside className="flex h-screen w-64 flex-col border-l border-[#E3DECF] bg-[#2A2E26] text-cream-soft">
      ...
      <div className="flex items-center gap-3 border-b border-white/10 px-6 py-5">
        <Image src="/Untitled-1.png" alt="مدى" width={48} height={28} className="h-10 w-auto brightness-0 invert opacity-90" />
        <div className="leading-tight">
          <span className="block text-xs font-bold text-cream-soft">لوحة التحكم</span>
          <span className="block text-[10px] text-cream-soft/50">إدارة المتجر</span>
        </div>
      </div>
            ...
            return (
              <li key={item.href}>
                ...
                >
                  <item.icon size={17} />
                  {item.label}
                  {active && <FiChevronLeft size={14} className="ms-auto" />}
                </Link>
              </li>
            );
          })}
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-semibold text-cream-soft">{admin?.name}</p>
            <p className="truncate text-xs text-cream-soft/50">{admin?.email}</p>
          </div>
          <button
            type="button"
